...
The native ABI declarations were referenced from WeFlow 5.0.0 (`electron/services/keyService.ts` and `wcdbCore.ts`). Upstream:To avoid disturbing the original author, it is not displayed here. Attribution: cc / WeFlow contributors. Complete supplied repository license: `licenses/WeFlow-LICENSE.txt` (CC BY-NC-SA 4.0).
...
The SQLCipher v4 layout was cross-checked against the supplied MemoTrace / WeChatMsg `wxManager/decrypt/decrypt_v4.py`, and message schema against `wxManager/db_v4/message.py`, contact schema against `contact.py`. Upstream: To avoid disturbing the original author, it is not displayed here. The local reader was implemented in JavaScript using Node.js crypto and SQLite WASM; the original Python modules are not bundled. This provenance must remain documented for any later licensing review.
