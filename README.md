...
2. 点击「检查程序」以便检查微信文件完整性。检查不会连接微信。
3. 选择 微信默认存储路径`C:\Users\你的用户名\xwechat_files` 或其下包含 `db_storage` 的账号目录，确认账号。
4. 粘贴已有密钥并点击「使用此密钥」；没有密钥时，请关闭微信以及其后台，并重新打开至微信登录界面，先点击软件中的「刷新窗口列表」，选正确的微信窗口，如：“有窗口·Weixin·11451”，再点击「开始获取按钮」，在这90秒的时间内登录微信，程序将自动抓取到密钥，然后点击「使用此密钥」。**每个用户只有一个密钥，该密钥可反复使用，复制密钥后请务必妥善保存，以便下次使用，多次获取密钥可能触发微信风控，风险自负**。
